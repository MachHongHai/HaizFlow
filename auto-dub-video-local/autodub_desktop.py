...
if "--douyin-channel-worker" in sys.argv:
    from autodub.services.douyin_channel_worker import main as run_douyin_channel_worker

    raise SystemExit(run_douyin_channel_worker())

if "--runtime-probe" in sys.argv:
    from autodub.core.runtime_probe import main as run_runtime_probe
...
if "--release-smoke" in sys.argv:
    from autodub.core.release_smoke import main as run_release_smoke

    smoke_index = sys.argv.index("--release-smoke")
    raise SystemExit(run_release_smoke(sys.argv[smoke_index + 1 :]))

if "--ui-smoke-test" in sys.argv:
    os.environ["AUTODUB_SMOKE_TEST"] = "1"
    from autodub.desktop.main import main as run_desktop_smoke
