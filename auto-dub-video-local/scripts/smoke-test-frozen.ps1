...
if (!$ArtifactPath) {
  $ArtifactPath = Join-Path $Root "dist\AutoDubVideoLocal"
}
$ArtifactPath = [System.IO.Path]::GetFullPath($ArtifactPath)
$Executable = Join-Path $ArtifactPath "AutoDubVideoLocal.exe"
