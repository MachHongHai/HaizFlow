...
$ArtifactPath = [System.IO.Path]::GetFullPath((Join-Path $DistRoot "AutoDubVideoLocal"))
...
$ArgsList += @("--hidden-import", "autodub.services.douyin_channel_worker")
$ArgsList += @("--hidden-import", "autodub.vendor.douyin_xbogus")
...
if ($IncludeCpuModel) {
  ...
  Invoke-PythonChecked -Arguments @(
    "-c", "import sys; sys.path.insert(0, r'$SourcePath'); from pathlib import Path; from autodub.core.model_integrity import verify_cpu_model; verify_cpu_model(Path(r'$ModelPath'))"
...
if ($IncludeGpuModel) {
  $SourcePath = Join-Path $Root "src"
  $ModelPath = & $Python -c "import sys; sys.path.insert(0, r'$SourcePath'); from autodub.config import MODELS_DIR; from pathlib import Path; print(Path(MODELS_DIR) / 'hymt2-transformers')"
  ...
  Invoke-PythonChecked -Arguments @(
    "-c", "import sys; sys.path.insert(0, r'$SourcePath'); from pathlib import Path; from autodub.core.model_integrity import verify_gpu_model; verify_gpu_model(Path(r'$ModelPath'))"
...
if (!(Test-Path -LiteralPath (Join-Path $ArtifactPath "AutoDubVideoLocal.exe") -PathType Leaf)) {
