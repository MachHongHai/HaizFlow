    ...
    while version < PROJECT_SCHEMA_VERSION:
        if version == 1:
            ...
            record.update(
                {
                    "schema_version": 2,
                    "project_id": str(record.get("project_id") or uuid.uuid5(uuid.NAMESPACE_URL, f"autodub:{key}")),
                    "key": key,
                    "project_name": name,
        ...
        if version == 3:
            ...
            project_id = str(record.get("project_id") or uuid.uuid5(uuid.NAMESPACE_URL, f"autodub:{legacy_key}"))
