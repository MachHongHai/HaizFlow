...
from autodub.schemas.channel_import import (
...
def _entry_candidate(platform: str, entry: dict) -> ChannelVideoCandidate | None:
...
def _hydrate_candidate(candidate: ChannelVideoCandidate, auth: dict) -> ChannelVideoCandidate | None:
    yt_dlp = _load_yt_dlp()
    options = _youtube_dl_options(auth)
    options["noplaylist"] = True
    with yt_dlp.YoutubeDL(options) as downloader:
        info = downloader.extract_info(candidate.source_url, download=False)
    if isinstance(info, dict) and _is_non_video_metadata(info):
        return None
    return _merge_candidate(candidate, info if isinstance(info, dict) else {})
...
def _scan_with_ytdlp(
    ...
    yt_dlp = _load_yt_dlp()
    scan_limit = request.scan_scope if request.ranking == "popular" else max(request.limit * 3, 60)
    collection_urls = (
        _youtube_collection_urls(request.url, request.duration_filter)
        if platform == "YouTube"
        else [request.url]
    )
    per_collection_limit = scan_limit
    if scan_limit and len(collection_urls) > 1:
        per_collection_limit = max(1, (scan_limit + len(collection_urls) - 1) // len(collection_urls))
    ...
    for collection_url in collection_urls:
        ...
        with yt_dlp.YoutubeDL(options) as downloader:
            info = downloader.extract_info(collection_url, download=False)
    ...
    for entry in entries:
        if cancel_event.is_set():
            raise DownloadCancelled("Channel inspection cancelled.")
        candidate = _entry_candidate(platform, entry)
        if not candidate:
            continue
    ...
    if to_hydrate:
        ...
        with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="channel-metadata") as executor:
            futures = {
                executor.submit(_hydrate_candidate, candidate, _auth_options(request)): candidate
...
def _douyin_worker_command() -> list[str]:
    ...
    return [sys.executable, "-m", "autodub.services.douyin_channel_worker"]
