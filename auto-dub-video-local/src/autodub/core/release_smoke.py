...
from autodub.core.paths import bundle_root, project_root
...
def run_release_smoke(*, require_cpu_model: bool, require_gpu_model: bool) -> dict[str, object]:
    ...
    _check((bundle / "autodub" / "desktop" / "qml" / "Main.qml").is_file(), "QML bundle", failures, details)
    ...
    if require_cpu_model:
        from autodub.config import HYMT2_CPU_MODEL_FILE
        from autodub.core.model_integrity import verify_cpu_model
    ...
    if require_gpu_model:
        from autodub.core.model_integrity import verify_gpu_model
