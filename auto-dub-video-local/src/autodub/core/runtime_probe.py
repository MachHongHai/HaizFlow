...
def run_runtime_probe(device: str) -> RuntimeProbeResult:
    ...
    os.environ["AUTODUB_PROCESSING_DEVICE"] = requested_device
    ...
    from autodub import config  # noqa: F401
...
def _probe_command(device: str) -> list[str]:
    ...
    return [sys.executable, "-m", "autodub.core.runtime_probe", "--child", device]
...
def probe_runtime(device: str, timeout_seconds: int = 240) -> RuntimeProbeResult:
    ...
    environment["AUTODUB_PROCESSING_DEVICE"] = requested_device
    environment.pop("AUTODUB_FORCE_CPU", None)
