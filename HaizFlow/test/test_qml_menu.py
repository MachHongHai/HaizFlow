...
ApplicationWindow {{
    ...
    def test_navigation_settings_and_project_page_actions_stay_uncluttered(self):
        ...
        downloads_position = main.index('text: I18n.t("Downloads")')
        single_position = main.index('text: I18n.t("Single")')
        batch_position = main.index('text: I18n.t("Batch")')
        publish_position = main.index('text: I18n.t("Social publishing")')
        self.assertLess(downloads_position, single_position)
        self.assertLess(single_position, batch_position)
        self.assertLess(batch_position, publish_position)
...
ApplicationWindow {{
    ...
    def test_translation_editor_auto_saves_and_tool_windows_have_no_minimize_control(self):
        ...
        self.assertIn("Keys.priority: Keys.BeforeItem", editor)
        self.assertIn("event.key === Qt.Key_Z || event.key === Qt.Key_Y", editor)
    ...
    def test_editor_preview_primes_a_frame_without_a_second_paused_seek(self):
        ...
        self.assertIn("visible: root.previewStatusVisible && root.previewUpdateBusy", editor)
    ...
    def test_social_publishing_uses_zernio_without_browser_automation(self):
        ...
        setup = (QML_DIR / "ZernioSetupPanel.qml").read_text(encoding="utf-8")
        access = (QML_DIR / "ZernioAccessPanel.qml").read_text(encoding="utf-8")
        ...
        combined = "\n".join((main, page, setup, guide, key_dialog, controller, service)).lower()

        self.assertIn('readonly property string routePublishProjects: "publish-projects"', main)
        self.assertIn("SocialPublishPage", main)
        self.assertIn("ZernioSetupPanel", page)
        self.assertIn('I18n.t("Social publishing")', setup)
        self.assertIn('I18n.t("Post options")', page)
        self.assertIn("zernioSetupPanel.openPostOptions()", page)
        self.assertIn("function openPostOptions()", setup)
        self.assertIn("ZernioAccessPanel", page)
        self.assertIn('I18n.t("Zernio guide and setup")', access)
        self.assertIn('I18n.t("Manage API key")', access)
        ...
        self.assertIn('I18n.t("Add videos")', page)
        self.assertIn('I18n.t("Add from projects")', page)
        self.assertIn("projectSourceDialog.openForSelection()", page)
        self.assertIn('I18n.t("From files")', page)
        self.assertIn('I18n.t("From folder")', page)
        ...
        self.assertLess(page.index("ZernioSetupPanel"), page.index('I18n.t("Default post text")'))
        self.assertLess(page.index('I18n.t("Default post text")'), page.index('I18n.t("Publishing queue")'))
        self.assertIn("Layout.preferredWidth: 3", page)
        self.assertIn("Layout.maximumHeight: 56", page)
        self.assertNotIn("Layout.fillHeight: true\n                Layout.preferredWidth", page)
        self.assertNotIn("Menu {", setup)
        self.assertNotIn("apiKeyManagementRequested", setup)
        self.assertIn("apiKeyManagementRequested", access)
        self.assertIn("disconnectZernioConnection", connections)
        self.assertIn('I18n.t("Disconnect platform")', connections)
        self.assertIn("onApiKeyManagementRequested: apiKeyDialog.openForConfiguration()", page)
        ...
        for platform in ("tiktok", "facebook", "instagram"):
            self.assertIn(f'root.platform === "{platform}"', setup)
        self.assertIn("zernioPrivacyLevels", setup)
        self.assertIn('I18n.t("Refresh")', connections)
        ...
        self.assertIn("publishConfirmation", page)
        ...
        self.assertNotIn("clearZernioApiKey", setup)
