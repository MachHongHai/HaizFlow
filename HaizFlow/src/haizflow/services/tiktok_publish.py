...
TIKTOK_BROWSER_DATA_DIR = os.path.join(RUNTIME_DATA_DIR, "browser-sessions", "tiktok")
...
def browser_session_directory() -> str:
    """Return the dedicated Chrome data directory that persists TikTok cookies."""
    return os.path.abspath(TIKTOK_BROWSER_DATA_DIR)


def clear_browser_session_data() -> bool:
    """Delete only HaizFlow's dedicated TikTok browser profile."""
    target = Path(browser_session_directory()).resolve()
    if target.name.casefold() != "tiktok" or target.parent.name.casefold() != "browser-sessions":
        raise RuntimeError("Refusing to clear an unsafe TikTok browser session path.")
...
def summarize(project_root: str) -> dict[str, Any]:
    ...
    if not items:
        ...
        status = "done"
    elif any(item["status"] == "awaiting_confirmation" for item in items):
        status = "processing"
    elif any(item["status"] in {"failed", "missing"} for item in items):
        status = "failed"
    else:
        status = "ready"
