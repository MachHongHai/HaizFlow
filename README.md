...
| Publish to TikTok | Prepare a project-backed publishing queue, connect a TikTok account through Zernio OAuth, choose account-supported privacy options, and publish or create drafts without browser automation. |
...
TikTok publishing is an optional cloud workflow. It requires a user-supplied Zernio API key and TikTok authorization through Zernio OAuth. Videos selected for publishing are uploaded to Zernio and TikTok. HaizFlow stores the API key in Windows Credential Manager, keeps publishing queues inside their projects, and does not use Playwright or automate the TikTok Studio web interface. The standard download and video-processing workflows do not require Zernio.
...
| Đăng TikTok | Chuẩn bị hàng đợi theo dự án, kết nối tài khoản TikTok qua OAuth của Zernio, chọn quyền riêng tư được tài khoản hỗ trợ, rồi đăng ngay hoặc tạo bản nháp mà không điều khiển trình duyệt. |
...
Đăng TikTok là quy trình cloud tùy chọn. Tính năng này cần API key Zernio do người dùng cung cấp và tài khoản TikTok được ủy quyền qua OAuth của Zernio. Video được chọn đăng sẽ tải lên Zernio và TikTok. HaizFlow lưu API key trong Windows Credential Manager, giữ hàng đợi trong từng dự án và không dùng Playwright hay tự động hóa giao diện web TikTok Studio. Các quy trình tải xuống và xử lí video thông thường không cần Zernio.
