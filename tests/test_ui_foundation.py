...
"""
    ...
    def test_manual_editor_keeps_subtitle_editing_out_of_the_visual_tool(self):
        ...
        visual_tool = (QML_DIR / "ManualImageToolPanel.qml").read_text(encoding="utf-8")
    ...
    def test_manual_source_and_audio_are_direct_controls(self):
        inspector = (QML_DIR / "ManualStageInspector.qml").read_text(encoding="utf-8")
        source_tool = (QML_DIR / "ManualSourceToolPanel.qml").read_text(encoding="utf-8")
        audio_tool = (QML_DIR / "ManualAudioToolPanel.qml").read_text(encoding="utf-8")
        image_tool = (QML_DIR / "ManualImageToolPanel.qml").read_text(encoding="utf-8")
        subtitle_tool = (QML_DIR / "ManualSubtitleToolPanel.qml").read_text(encoding="utf-8")
        ...
        self.assertIn('inspector.hasCurrentCache("voice")', audio_tool)
        self.assertNotIn("nghe thử", audio_tool.lower())
        self.assertNotIn("AudioMixDialog", inspector)
        self.assertIn("onCurrentStageChanged:", inspector)
        self.assertIn("inspectorScroll.contentY = 0", inspector)
        ...
        self.assertIn('qsTr("Mở rộng")', subtitle_tool)
        subtitle_dialog = (QML_DIR / "ManualSubtitleEditorDialog.qml").read_text(encoding="utf-8")
        self.assertIn("preferredWidth: 900", subtitle_dialog)
        self.assertIn("preferredHeight: 590", subtitle_dialog)
        ...
        text_style_bar = (QML_DIR / "CompactTextStyleBar.qml").read_text(encoding="utf-8")
        ...
        self.assertIn("id: previewDebounce", transform_overlay)
        self.assertIn("interval: 120", transform_overlay)
        self.assertIn("HoverHandler {", scale_handle)
        ...
        self.assertIn("chooseWatermarkVideo", image_tool)
        self.assertNotIn("TextStyleLibraryDialog", inspector)
        self.assertNotIn("availableFontFamilies", text_style_bar)
        self.assertIn("TextColorStrip", text_style_bar)
        self.assertIn('property string watermarkKind: "text"', watermark_overlay)
        self.assertIn("watermarkImageSource", watermark_overlay)
        self.assertIn("watermarkVideoSource", watermark_overlay)
        self.assertIn("MediaPlayer {", watermark_overlay)
        ...
        self.assertIn("TrackHeader {", timeline)
        self.assertIn("Flickable.AutoFlickDirection", timeline)
        self.assertIn("ScrollBar.vertical:", timeline)
        self.assertIn('root.trackStateRequested(', timeline)
    ...
    def test_manual_subtitle_editor_dialog_is_compact_and_responsive(self):
        script = f"""
...
assert dialog.property('height') == 590
assert editor.property('height') >= 300
window.close()
window.deleteLater()
app.processEvents()
"""
...
for stage in range(8):
    ...
    assert float(scroll.property("contentHeight")) >= float(loader.property("height"))
...
"""
    ...
    def test_manual_export_completion_is_visible_and_loaded_on_demand(self):
        ...
        self.assertIn('text: qsTr("Mở video xuất")', toolbar)
        self.assertIn("onOutputRequested: AppController.openOutputFile()", workspace)
    ...
    def test_dialogs_share_chrome_and_about_links_use_one_alignment(self):
        ...
        self.assertIn('title: qsTr("Chỉnh nội dung phụ đề")', subtitle_dialog)
